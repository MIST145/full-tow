...
-- ###############################################
    ...
    function ScoopTowTruck:Thread_ProcessGrabHookStrapVeh(data)
        ...
        if data.isCarHooked then
            ...
            if IsControlJustPressed(0, 51) then
                if data.isHookInUse then
                    self:PlayMechanicAnimation(data.playerPed)
...
-- ##################################
    ...
    function ScoopTowTruck:PlayMechanicAnimation(ped)
...
-- ########################################
    ...
    function ScoopTowTruck:GrabHookAsync(ped)
        ...
        local runFunc = function()
            -- Play mechanic animation
            self:PlayMechanicAnimation(ped)
...
-- ########################################
    ...
    function ScoopTowTruck:AttachCarToBed()
        ...
        local attachPos = self.config.bedAttach.offset + vector3(0.0, 0.0, carPos.z - bedPos.z)
